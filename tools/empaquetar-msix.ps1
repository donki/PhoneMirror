<#
.SYNOPSIS
    Construye el paquete MSIX de sOC Phone Mirror para la Microsoft Store.
...
param(
    ...
    [string] $IdentityName = "sOCratic.sOCPhoneMirror",
    ...
    [string] $DisplayName = "sOC Phone Mirror",
    [string] $Version,
    [switch] $Autofirmar
)
...
$proyecto = Join-Path $raiz "PhoneMirror.csproj"
$origenPaquete = Join-Path $raiz "Package"
$trabajo = Join-Path $raiz "bin\msix"
$salida = Join-Path $raiz "bin\sOCPhoneMirror.msix"
...
Copy-Item (Join-Path $publicado "sOCPhoneMirror.exe") $trabajo
Copy-Item (Join-Path $publicado "Assets") $trabajo -Recurse
...
if ($Autofirmar) {
    ...
    $cert = New-SelfSignedCertificate -Type Custom -Subject $Publisher `
        -KeyUsage DigitalSignature -FriendlyName "Phone Mirror (solo pruebas)" `
